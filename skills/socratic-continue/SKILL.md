...
description: Use when the user invokes /socratic-continue — resume the MOST RECENT /socratic debate in the current session with the SAME two debaters, feed them a new idea/point/objection to chew on, run more clash rounds, and re-translate the outcome. Requires a prior /socratic (or /socratic-continue) run earlier in THIS session — the debaters live in the moderator's live agent handles only, so a fresh session with no prior run cannot resume them. A bare /socratic always starts a NEW debate from scratch; use this only to continue an existing one with fresh input.
---

# socratic-continue — throw a new bone into the last debate

Resume the **most recent `socratic` debate in the current session** with the **same two
debaters**, feeding them a new idea the user wants chewed on. You are still the **moderator**:
...
**Announce at start:** "Using socratic-continue — resuming the last debate with the same two
...
- **Continuation, not restart.** You must resume the SAME `socratic-alpha` / `socratic-beta`
  agents (by name or by the `agentId` from their spawn). Re-spawning fresh agents would discard
  their context and re-collapse the two samples into an echo — that is a NEW debate, not a
...
evidence both must answer.

## 2. Locate the prior debate and its two agents

From this session's context, recover the two debater agents from the most recent `socratic` (or
prior `socratic-continue`) run — their names (`socratic-alpha`, `socratic-beta`) and/or their
`agentId`s. You will `SendMessage` to those exact handles.
...
question instead. Never silently spin up two new agents and pass it off as a continuation (that is
...
**do not resend the brief.** In a single message, `SendMessage` to **both** debaters with the same
...
- Then the new point, in the same **dense shorthand** the debate runs in — one claim per line,
  ...
  never feed the two agents materially different ideas.
- Ask each to **restate POS** after addressing it.

Send both in one message so they run concurrently. Collect **both** replies before judging.

## 4. Run the continuation loop

**Rounds — clash.** After the injection round, relay verbatim exactly as in `socratic` §4 (dense
in, dense out — do not expand between debaters):
...
(Both in one message; concurrent. Collect both before judging.)

After **every** round, evaluate the stop conditions from `socratic` §5 — **consensus**, **stall**
(both `HOLD:` same round), **loop** (a rebutted claim re-raised unchanged) — reading the tags then
...
- **Did it move the needle?** State plainly whether the new point shifted either position, broke a
...
- **Updated recommendation / crux** — the current bottom line after the injection: the new agreed
  answer (on consensus), or the sharpened `CRUX:` and both `POS:` lines (on no consensus), stated
  fairly.
- **New concessions & tradeoffs** — anything either side granted or flagged in response to the
  point.
...
- **Outcome tag** — how this continuation ended (`consensus` / `stall` / `loop` / `cap`), the
  number of continuation rounds, and a note that it was a `continuation` of the prior run.
...
- **A debater returns nothing / dies on resume** — resend once. If it still fails, continue with
  the surviving debater's view and **say so** in the synthesis; a one-sided continuation is honest,
  a faked two-sided one is not.
...
- The debaters inherit the model and stances of the original run — no re-differentiation needed,
  ...
  than a restart.
- A continuation is cheap precisely because the expensive part (building both cases from scratch)
